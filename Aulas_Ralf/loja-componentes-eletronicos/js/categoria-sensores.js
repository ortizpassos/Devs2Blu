// JavaScript específico para página de sensores
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    produtosFiltrados = produtos.filter(produto => produto.categoria === 'sensores');
    exibirProdutos();
        ...
        <div class="col-lg-3 col-md-4 col-sm-6 mb-4">
            <div class="card card-produto ${cardClass}">
                <div class="position-relative">
                    <span class="badge-categoria">Sensor</span>
...
// Função para filtrar por categoria (sensores)
...
// Função para filtrar por tipo de sensor
function filtrarPorTipo(tipo) {
    ...
    const botaoTipo = document.querySelector(`[onclick="filtrarPorTipo('${tipo}')"]`);
    if (botaoTipo) {
        botaoTipo.classList.add('active');
    }
...
function aplicarFiltros() {
    let produtosFiltradosTemp = produtos.filter(produto => produto.categoria === 'sensores');
    
    // Filtrar por tipo
    if (filtroTipoAtual) {
        produtosFiltradosTemp = produtosFiltradosTemp.filter(produto => {
            const nomeLower = produto.nome.toLowerCase();
            switch (filtroTipoAtual) {
                case 'temperatura':
                    return nomeLower.includes('temperatura') || nomeLower.includes('dht');
                case 'movimento':
                    return nomeLower.includes('pir') || nomeLower.includes('movimento') || nomeLower.includes('vibração') || nomeLower.includes('acelerômetro');
                case 'distancia':
                    return nomeLower.includes('ultrassônico') || nomeLower.includes('hc-sr04') || nomeLower.includes('distância');
                default:
                    return true;
            }
        });
...
// Reutilizar funções do carrinho e outras funcionalidades do arquivo anterior
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <p><strong>Categoria:</strong> Sensor</p>
